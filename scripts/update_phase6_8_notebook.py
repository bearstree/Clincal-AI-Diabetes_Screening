...
Pinned 2026 build foundation: Android Gradle Plugin 9.3.0, Gradle 9.5.0, JDK 17, compile/target SDK 37, Kotlin/Compose compiler 2.3.21, Compose BOM 2026.06.01. These versions follow current official Android compatibility/setup documentation. Debug uses the emulator-only host address `http://10.0.2.2:8000` and a debug-only cleartext exception. Release builds require an HTTPS API URL.
"""
            ),
            code(
                """
...
"""
            ...
            markdown(
                """
Local Android compilation was not available on this workstation because Gradle, Android SDK tools, and ADB are not installed. `.github/workflows/android.yml` supplies JDK 17, Android SDK 37/build-tools 36.0.0, and Gradle 9.5.0 to compile the debug APK on every Android change. This is an explicit environment limitation, not a claimed local build result.
"""
            ),
            markdown(
                """
